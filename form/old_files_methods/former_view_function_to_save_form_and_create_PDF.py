def post(self, request, *args, **kwargs):
    ...
    if form.is_valid():
        ...
        # Fairly groomed
        pdf.rect(x=215, y=580, width=10, height=10)test
        ...
        if form_to_save.appetite_change:
        ...
        if form_to_save.diagnostic_impression:
            pdf.drawString(50, 140, form_to_save.diagnostic_impression)
        ...
        if form_to_save.current_medication:
            pdf.drawString(50, 95, form_to_save.current_medication)
        ...
        if form_to_save.discussion_treatment:
            pdf.drawString(50, 65, form_to_save.discussion_treatment)

        # Signature
        pdf.setFont('Helvetica-Bold', 8)
        pdf.drawString(30, 30, 'Signature:')
        pdf.setFont('Helvetica', 8)
        if form_to_save.signature:
            pdf.drawString(80, 30, form_to_save.signature)
